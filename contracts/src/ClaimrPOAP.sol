...
/**
 * @title ClaimrPOAP
 * @notice Soulbound proof-of-attendance NFTs for Monad events.
 *
 * Eligibility (geofencing, email/identity verification) is checked off-chain by
 * Claimr's backend. After a successful verification the backend — holding
 * MINTER_ROLE — mints a POAP directly to the attendee's wallet, so claiming is
 * gasless for the user.
 *
 * Invariants:
 *  - One token per (eventId, wallet).
 *  - Each token stores its eventId and a metadata URI.
 *  - Tokens are soulbound (non-transferable) — a POAP is proof *you* were there.
 */
contract ClaimrPOAP is ERC721URIStorage, AccessControl {
    ...
    /// @notice eventId => number of POAPs minted.
    ...
    constructor(address admin, address minter)
        ERC721("Claimr Proof of Presence", "CLAIMR")
    {
    ...
    /**
     * @notice Mint a POAP to a verified attendee.
    ...
    /// @notice Total POAPs minted across all events.
