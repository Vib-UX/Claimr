...
export function ClaimPanel({ event }: { event: ClaimrEvent }) {
  const { authenticated, email, wallet, ready, getAccessToken } = useClaimrAuth();
  const claims = useClaimrStore((s) => s.claims);
  const addClaim = useClaimrStore((s) => s.addClaim);
    ...
    <div className="rounded-3xl border border-border bg-card p-5 card-glow sm:p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-display font-semibold leading-none">Claim your moment</p>
      ...
      {claimBlocked && existingClaim ? (
        <div className="mt-5 rounded-2xl border border-[color-mix(in_oklab,var(--success)_40%,transparent)] bg-[color-mix(in_oklab,var(--success)_7%,transparent)] p-4 text-center">
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Your proof of presence is minted onchain.
          </p>
          <a
          ...
          {minting ? (
            ...
            <motion.div layout className="mt-5">
              ...
              >
                ...
                {eligible ? "Capture the moment" : "Complete steps to claim"}
              </Button>
              {mintStatus === "error" && (
              ...
              <p className="mt-3 text-center text-xs text-muted-foreground">
                Gas sponsored on Monad Testnet · proof you were there
              </p>
            </motion.div>
          )}
        </>
