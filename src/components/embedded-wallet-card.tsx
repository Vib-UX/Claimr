...
export function EmbeddedWalletCard() {
  const { email, wallet, embedded } = useClaimrAuth();
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-[linear-gradient(135deg,color-mix(in_oklab,var(--monad)_14%,var(--card)),var(--card))] p-5">
      <div
        className="pointer-events-none absolute -right-10 -top-10 size-40 rounded-full opacity-40 blur-2xl"
        style={{ background: "var(--monad)" }}
      />
