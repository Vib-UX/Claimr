...
export function VenueMap({ venue }: { venue: Venue }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-border">
      <div className="relative h-52 bg-[linear-gradient(135deg,color-mix(in_oklab,var(--monad)_10%,var(--muted)),var(--muted))]">
        {/* Grid streets */}
        ...
        <div className="absolute inset-0 opacity-30 [background-image:linear-gradient(115deg,transparent_46%,color-mix(in_oklab,var(--monad)_35%,transparent)_47%,color-mix(in_oklab,var(--monad)_35%,transparent)_53%,transparent_54%)]" />

        {/* Geofence radius */}
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="relative flex items-center justify-center">
            <span className="absolute size-36 rounded-full border border-[color-mix(in_oklab,var(--monad)_50%,transparent)] bg-[color-mix(in_oklab,var(--monad)_12%,transparent)]" />
            <span className="absolute size-36 animate-ping rounded-full border border-[color-mix(in_oklab,var(--monad)_40%,transparent)] [animation-duration:3s]" />
